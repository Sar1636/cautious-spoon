...
#     """
#     Two-step pipeline:
#       Step 1 preprocess — reads raw CSV (URI_FILE), writes cleaned_data.csv to output folder
#       Step 2 train      — reads cleaned folder (URI_FOLDER), writes model to output folder

#     Input/output names must match the ${{inputs.X}} / ${{outputs.X}} tokens in commands.
#     """
...
# ── Azure ML Client ───────────────────────────────────────────────────
def get_ml_client():
    # ✅ FIX: read env variables INSIDE function not at module level
    tenant_id       = os.environ["AZURE_TENANT_ID"]
    subscription_id = os.environ["AZURE_SUBSCRIPTION_ID"]
    client_id       = os.environ["AZURE_CLIENT_ID"]
    client_secret   = os.environ["AZURE_CLIENT_SECRET"]
